...
dft2 = conn.query(
    "select sample.tag as sample_tag, trait.uploaddate, sample.collectiondate, trait.responsible_id, trait.tag as trait_tag, sample.family, sample.genus, sample.species, sample.nomenclature, sample.parent_id from individualtrait NATURAL JOIN trait INNER JOIN sample ON sample.id = trait.samples_id"
)
try:
